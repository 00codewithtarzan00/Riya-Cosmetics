  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden animate-fade-in">
      ...
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-lg bg-white shadow-2xl flex flex-col justify-between border-l border-stone-200 relative">
          
          {/* Custom Support Overlay (with zero external logos or branding) */}
          {supportOrder && (
            <div className="absolute inset-0 bg-white z-20 flex flex-col justify-between animate-fade-in">
              {/* Header */}
              <div className="p-5 border-b border-stone-100 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <MessageSquare className="w-5 h-5 text-[#ff0052]" />
                  <div>
                    <span className="text-sm font-extrabold uppercase tracking-widest text-stone-900 block leading-tight">
                      Order Support / सहायता
                    </span>
              ...
              <div className="flex-grow overflow-y-auto p-5 space-y-6">
                {/* Official support banner with no photo/name */}
                <div className="p-4 bg-stone-50 border border-stone-200/80 text-center">
                  <p className="text-xs font-bold text-stone-900 uppercase tracking-wide">
                    Contact Riya Cosmetics Official Store
                  </p>
                </div>

                {/* Ticket Details summary card */}
                <div className="p-4 border border-stone-100 bg-stone-50/50 space-y-2.5">
                  <span className="text-[9px] font-mono tracking-widest text-stone-400 font-bold uppercase block">Support Ticket Details</span>
                  <div className="flex justify-between text-xs">
                    <span className="text-stone-500 font-medium">Order ID:</span>
                    <span className="font-mono font-bold text-stone-900">#{supportOrder.id}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-stone-500 font-medium">Customer:</span>
                    <span className="font-bold text-stone-900">{supportOrder.customerName}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-stone-500 font-medium">Status:</span>
                    <span className="font-bold text-stone-900">{supportOrder.status}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-stone-500 font-medium">Grand Total:</span>
                    <span className="font-mono font-bold text-[#ff0052]">₹{supportOrder.totalAmount.toLocaleString('en-IN')}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2.5 pt-2">
                  <button
                    onClick={() => handleSendWhatsAppSupport(supportOrder)}
                    className="w-full flex items-center justify-center gap-2 py-3 bg-[#ff0052] hover:bg-[#ff0052]/90 text-white font-extrabold text-[10px] uppercase tracking-widest transition-all duration-300 rounded-none cursor-pointer"
                  >
                    <MessageSquare className="w-4 h-4" /> Message Support Team
                  </button>

                  <button
                    onClick={() => {
                      const text = `Order ID: #${supportOrder.id}\nCustomer: ${supportOrder.customerName}\nStatus: ${supportOrder.status}\nTotal: ₹${supportOrder.totalAmount.toLocaleString('en-IN')}`;
                      navigator.clipboard.writeText(text);
                      setCopiedDetails(true);
                      setTimeout(() => setCopiedDetails(false), 2000);
                    }}
                    className="w-full flex items-center justify-center gap-2 py-3 bg-white border border-stone-200 hover:border-stone-900 text-stone-800 font-extrabold text-[10px] uppercase tracking-widest transition-all duration-300 rounded-none cursor-pointer"
                  >
                    {copiedDetails ? 'Ticket Copied!' : 'Copy Support Details'}
                  </button>
                </div>
              </div>
              ...
              <div className="p-4 bg-stone-50 border-t border-stone-100 text-center">
                <p className="text-[9px] uppercase font-mono tracking-widest text-stone-400 font-bold">
                  Riya Cosmetics Official Support Center
                </p>
              </div>
            </div>
          )}
                  ...
                  >
                    ...
                    <div className="mt-2 pt-2 border-t border-stone-100/50">
                      <a
                        ...
                        onClick={onClose}
                        className="w-full flex items-center justify-center gap-1.5 py-2 border border-stone-200 hover:border-stone-900 bg-white text-stone-700 hover:text-stone-900 font-bold text-[9px] uppercase tracking-wider transition-all"
                      >
