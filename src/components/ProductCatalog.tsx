...
export default function ProductCatalog({
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    return result;
  }, [products, selectedCategory, searchQuery, sortBy, wishlist]);
                            ...
                            <div className="flex-grow min-w-0 flex flex-col justify-between">
                              ...
                              <div className="flex items-center justify-between mt-1.5">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className="flex items-center border border-stone-200 rounded-none bg-white shadow-xs">
                                    <button
                                      onClick={() => onUpdateCartQuantity(item.product.id, item.quantity - 1)}
                                      className="w-11 h-11 sm:w-8 sm:h-8 flex items-center justify-center text-stone-500 hover:bg-stone-50 active:bg-stone-100 transition-colors cursor-pointer select-none"
                                      aria-label="Decrease quantity"
                                    >
                                      {item.quantity === 1 ? <Trash2 className="w-4 h-4 sm:w-3.5 sm:h-3.5 text-red-500" /> : <Minus className="w-4 h-4 sm:w-3.5 sm:h-3.5" />}
                                    </button>
                                    <span className="px-3 min-w-[28px] text-center text-xs font-black font-mono text-stone-900 select-none">
                                      {item.quantity}
                                    </span>
                                    <button
                                      onClick={() => onUpdateCartQuantity(item.product.id, item.quantity + 1)}
                                  ...
                                  <button
                                    ...
                                    className="w-11 h-11 sm:w-8 sm:h-8 flex items-center justify-center text-stone-400 hover:text-red-600 hover:bg-red-50 border border-stone-100 hover:border-red-200 transition-all cursor-pointer"
